...
# Kenya Payroll Rates record has, so it doesn't need to change if this does. Only
...
class KenyaPayrollRates(Document):
	...
	the generator (not yet built) reads a Kenya Payroll Rates record and writes
	...
	def get_effective(as_of_date=None):
		"""Return the name of the Kenya Payroll Rates record in force on a given date.
