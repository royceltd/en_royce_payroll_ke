...
// below is explicit about which Kenya Payroll Rates record it acts on
...
frappe.ui.form.on("Kenya Payroll Rates", {
	refresh(frm) {
		...
		// whoever can already write Kenya Payroll Rates itself (frm.perm mirrors the
		// same has_permission("Kenya Payroll Rates", "write") check setup.py makes
...
function regenerate_components(frm) {
	frappe.confirm(
		...
		() => {
			frappe.call({
				method: "royce_payroll_ke.kenya_payroll.setup.regenerate",
...
function verify_company(frm) {
	frappe.prompt(
		...
		(values) => {
			frappe.call({
				method: "royce_payroll_ke.kenya_payroll.setup.verify",
				args: { company: values.company, rates: frm.doc.name },
				freeze: true,
