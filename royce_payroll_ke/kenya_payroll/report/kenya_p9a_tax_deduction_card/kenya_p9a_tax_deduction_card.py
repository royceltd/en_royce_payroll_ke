...
from royce_payroll_ke.kenya_payroll.report_utils import card_type_sums
...
def month_rows(employee, company, year_start, year_end):
	...
	while month_date <= year_end:
		...
		rates_name = KenyaPayrollRates.get_effective(month_end)
		personal_relief = flt(frappe.db.get_value("Kenya Payroll Rates", rates_name, "personal_relief")) if rates_name else 0
...
def download_certificate(company, fiscal_year, employee):
	...
	html = frappe.render_template(
		"royce_payroll_ke/kenya_payroll/report/kenya_p9a_tax_deduction_card/kenya_p9a_tax_deduction_card.html",
		{
			"company": company,
			"employee": employee,
