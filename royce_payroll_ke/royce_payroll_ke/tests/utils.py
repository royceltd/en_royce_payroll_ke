...
can have a real, committed Payroll Rates record (or several) sitting in the
...
from royce_payroll_ke.royce_payroll_ke.setup import provision
...
def make_test_payroll_rates(effective_from, submit=True, **overrides):
	"""Build a Payroll Rates record for tests. Feb 2026 KRA numbers by
	...
	(_get_rates_doc() -> frappe.get_doc("Payroll Rates", name)), which is
	...
	doc = frappe.get_doc({"doctype": "Payroll Rates", "effective_from": effective_from, **values})
	doc.insert(ignore_permissions=True)
	if submit:
		doc.submit()
	return frappe.get_doc("Payroll Rates", doc.name)
		...
		holiday_list = frappe.get_doc(
			{
				...
				"holiday_list_name": f"Royce Payroll Ke Test Holidays {frappe.generate_hash(length=8)}",
